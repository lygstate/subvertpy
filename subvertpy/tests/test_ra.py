...
class TestRemoteAccess(SubversionTestCase):
    ...
    def test_iter_log(self):
        def check_results(returned):
            ...
            if ra.api_version() < (1, 6):
                self.assertEquals({'/foo': (b('A'), None, -1, NODE_UNKNOWN)}, paths)
            else:
                self.assertEquals({'/foo': (b('A'), None, -1, NODE_DIR)}, paths)
    ...
    def test_get_log(self):
        ...
        def check_results(returned):
            ...
            if len(returned[1]) == 3:
                ...
                (paths, revnum, props, has_children) = returned[1]
            self.assertEquals({'/foo': (b('A'), None, -1)}, paths)
            self.assertEquals(revnum, 1)
            self.assertEquals(set(["svn:date", "svn:author", "svn:log"]), 
                              set(iterkeys(props)))
    ...
    def test_get_file(self):
        ...
        stream.seek(0)
        self.assertEquals(b("a"), stream.read())
...
class AuthTests(TestCase):
    ...
    def test_simple(self):
        ...
        self.assertEquals(("foo", "geheim", 0), next(creds))
    ...
    def test_username(self):
        ...
        self.assertEquals(("somebody", 0), next(creds))
        self.assertRaises(StopIteration, next, creds)
    ...
    def test_client_cert(self):
        ...
        self.assertEquals(("filename", False), next(creds))
        self.assertRaises(StopIteration, next, creds)
    ...
    def test_client_cert_pw(self):
        ...
        self.assertEquals(("supergeheim", False), next(creds))
        self.assertRaises(StopIteration, next, creds)
    ...
    def test_server_trust(self):
        ...
        self.assertEquals((42, 0), next(creds))
        self.assertRaises(StopIteration, next, creds)
    ...
    def test_retry(self):
        ...
        creds = auth.credentials("svn.username", "MyRealm")
        self.assertEquals(("somebody1", 0), next(creds))
        self.assertEquals(("somebody2", 0), next(creds))
        self.assertEquals(("somebody3", 0), next(creds))
        self.assertRaises(StopIteration, next, creds)
